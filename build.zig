...
const zero_native_cli = "node_modules/.bin/zero-native";
const app_exe_name = "co-translator";
...
pub fn build(b: *std.Build) void {
    ...
    const dev = b.addSystemCommand(&.{ zero_native_cli, "dev", "--manifest", "app.zon", "--binary" });
    ...
    const package_output = b.fmt("zig-out/package/{s}-0.0.1-{s}-{s}{s}", .{ app_exe_name, @tagName(package_target), optimize_name, packageSuffix(package_target) });
    const clean_package = b.addSystemCommand(&.{ "rm", "-rf", package_output });
    ...
    const chmod_package = b.addSystemCommand(&.{ "chmod", "+x", b.fmt("{s}/Contents/MacOS/{s}", .{ package_output, app_exe_name }) });
    chmod_package.step.dependOn(&package.step);
    const package_step = b.step("package", "Create a local package artifact");
    package_step.dependOn(&chmod_package.step);
    ...
    if (platform == .macos) {
        switch (web_engine) {
            ...
            .chromium => {
                ...
                if (cef_auto_install) {
                    const cef_auto = b.addSystemCommand(&.{ zero_native_cli, "cef", "install", "--dir", cef_dir });
